...
# Generic values
_map_code = _T("Overwatch share code")
_user = _T("User name")
...
add_guide_args = {_map_code_a: _map_code, "url": _T("Valid URL to guide (YouTube, Streamable, etc)")}
...
remove_record = _Desc(name=_T("remove-record"), description=_T("Remove a record from a user"))
...
leaderboard = _Desc(name=_T("leaderboard"), description=_T("View leaderboard of any map in the database."))
leaderboard_args = {_map_code_a: _map_code, _level_name_a: _level_name, "verified": _T("Only show fully verified video submissions")}

personal_records = _Desc(name=_T("personal-records"), description=_T("View your (by default) personal records or another users)"))
personal_records_args = {_user_a: _user, "wr_only": _T("Only show world records, if any")}
